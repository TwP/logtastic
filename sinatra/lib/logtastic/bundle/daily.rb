...
class Logtastic::Bundle::Daily < Mongo::Collection

  attr_reader :bundle

  def initialize( bundle )
    @bundle = bundle
    super(database, bundle.name + '.daily')
  end
  ...
  def rollup
    time = latest
    now = Time.now.utc
    return self if (now.to_i - (time.to_i + 1.day)) < 10.minutes

    now = now.change(:hour => 0)
    docs = @bundle.hourly.daily(time, now)
...
end  # class Logtastic::Bundle::Daily
